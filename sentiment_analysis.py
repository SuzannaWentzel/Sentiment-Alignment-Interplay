...
fig_best, ax_best = plt.subplots(figsize=(15, 10))
fig_best.tight_layout()
...
for d_idx in discussion_ids_with_class:
    discussion = discussions_in_bin_length.loc[discussions_in_bin_length['discussion_id'] == d_idx]
    ax_best.plot(discussion['time_post_id'], discussion['rolling_average'], linewidth=0.7, alpha=0.3, color='#d74a94')

fig_best.savefig(f'Results/Frontpage/sentiment_bin{i_bin+1}_class{u_class+1}')
fig_best.show()
...
fig_best, ax_best = plt.subplots(figsize=(5, 5))
fig_best.tight_layout()
fig_best.subplots_adjust(left=0.2, right=0.9, top=0.9, bottom=0.1)
...
ax_best.set_ylim((-0.75, 0.75))
ax_best.set_xlim((0, bin_length[1]))
for d_idx in discussion_ids_with_class:
    discussion = discussions_in_bin_length.loc[discussions_in_bin_length['discussion_id'] == d_idx]
    ax_best.plot(discussion['time_post_id'], discussion['rolling_average'], linewidth=0.5)
trend = discussions_pivoted_df_with_class.mean()
mean_per_post = trend.reset_index()
ax_best.plot(mean_per_post['time_post_id'], mean_per_post[0], linestyle='dashed', color='black')

ax_best.set_ylabel('Sentiment score')
ax_best.set_xlabel('Posts in time')

fig_best.suptitle(f'Sentiment over time for class {u_class+1}')
fig_best.savefig(f'Results/Poster/sentiment_bin_{i_bin+1}_class_{u_class+1}')
fig_best.show()
